...
public class SmsYueYangUtil {
	...
	public static String endpoint = ConfigManager.getInstance().getConfig("sms_server_yueyang_url");
	public static String applicationid = ConfigManager.getInstance().getConfig("sms_server_yueyang_applicationid");
	public static String username = ConfigManager.getInstance().getConfig("sms_server_yueyang_username");
	public static String password = ConfigManager.getInstance().getConfig("sms_server_yueyang_password");
	public static String orgcode = ConfigManager.getInstance().getConfig("sms_server_yueyang_orgcode");
	public static String orgname = ConfigManager.getInstance().getConfig("sms_server_yueyang_orgname");
	public static String key = ConfigManager.getInstance().getConfig("sms_server_yueyang_key");
	

	//短信验证码接口调用webservices			岳阳
	public static String sendMessageWebserviceInfo(String content, String phone) {
		...
		try { 
			...
			msbean.setReqdeliveryreport("0");
			
			call.setOperation("sendSingleSms");//调用的是 单条短信发送给若干个手机号码的接口
			String send = (String)call.invoke(new Object[]{username,password,applicationid,orgcode,orgname,msbean});  
			returnvalue=send;
