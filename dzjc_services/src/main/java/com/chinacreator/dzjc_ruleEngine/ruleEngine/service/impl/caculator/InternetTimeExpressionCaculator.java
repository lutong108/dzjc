 ...
 * @date 2018-8-1
 */
public class InternetTimeExpressionCaculator implements RuleEngineCaculatorServiceIfc {

	// 网上办事默认时限。
	private static String internet_time_limit = ConfigManager.getInstance().getConfig("internet_time_limit");
	
	/**
	 * 网上办事时限监察 算法： 1、对于网上提交的办件，以受理时间减提交时间，计算超出时间 2、计算超出时间时，必须去掉节假日、特别程序时间
	 * 
	 * @param elemId
	 * @throws Exception
	 */
	...
	public String doCaculator(String elementId,String batchId,List<JcAllbussinesSum> list1) throws Exception {
		...
		try {
			...
			for(int i=0; i<supSize; i++){
				...
				String submitTime = ft.format(list.get(i).getSubmitTime());
				String finishTime = "";
				...
				try {
					...
					exceptions.append("businessType：5，").append(instanceId).append("：")
				...
				String rltMemo = "网上办事时限超过承诺时限，超过" + dateInv + "天";
				//将处理后的结果集封装
				...
				resultElement.setBusinessType("5");
			...
			System.err.println("网上办事发牌规则执行失败," + e.getMessage());
			e.printStackTrace();
