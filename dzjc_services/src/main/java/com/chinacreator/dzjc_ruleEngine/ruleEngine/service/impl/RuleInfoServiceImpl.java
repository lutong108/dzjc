...
import com.chinacreator.dzjc_ruleEngine.ruleEngine.dao.impl.RuleInfoDaoImpl;
...
/**
 * @author created by 李浪 2018-3-30 上午10:40:17
 */
public class RuleInfoServiceImpl implements RuleInfoServiceIfc {

	@Override
	public List<EnginecfgBean> getCfgValueList() {
		RuleInfoDao dao = new RuleInfoDaoImpl();

		return dao.getCfgValueList();
	}

	@Override
	public RuleWithBusinessBean getRuleWithBusinessBeanList(List<JcAllbussinesSum> list1) {
		RuleInfoDao dao = new RuleInfoDaoImpl();
	...
	@Override
	public RuleWithBusinessBean getRuleWithBusinessConsultList(List<JcAllbussinesSum> list1) {
		RuleInfoDao dao = new RuleInfoDaoImpl();
		List<RuleBaseinfoBeanAll> list = dao.getRuleBaseinfoBeanConsultList(list1);
		return processList(list);
	}

	public RuleWithBusinessBean processList(List<RuleBaseinfoBeanAll> list) {
		...
		if (list != null && list.size() > 0) {
		    ...
		    /*for (JcAllbussinesSum jcAllbussinesSum : list1) {
		    	 String bussinessId = jcAllbussinesSum.getBussinessId();
		    	 String bussinessType = jcAllbussinesSum.getBussinessType();
		    	 bussinessTypeMap.put(bussinessId, bussinessType);
;			}*/
		     
			/*for (int i = 0; i < list.size(); i++) {
				list.get(i).getAreaCode();
			}*/
			...
			for (int i = 0; i < list.size(); i++) {
				...
				String streetOrgId=list.get(i).getStreetOrgId();
				...
				approveId=list.get(i).getApproveId();
				...
				if(!businessLogMap.containsKey(businessid+"#"+superviselevelcode)){
					...
					logMap.put("streetOrgId", streetOrgId);
