...
/**
 * <p>Title: 电子监察系统</p>
 * <p>Description:表达式计算（办理时限监察）实现类</p>
 * <p>Company:湖南科创</p>
 * @author 刘尹
 * @version 1.0
 * @date 2011-9-1
 */
public class ProcessTimeExpressionCaculator implements RuleEngineCaculatorServiceIfc {
	
	//private static final Logger LOG = Logger.getLogger(ProcessTimeExpressionCaculator.class.getName());
	...
	public String doCaculator(String elemId,String batchId,List<JcAllbussinesSum> list1) throws Exception {
		...
		if(idList.size() > 0){
			map.put("list", idList);
			DaoFactory.create(ResultElement.class).getSession().insert("com.chinacreator.dzjc_ruleEngine.ruleEngine.entity.ResultElementMapper.insertProcessTime",map);
		}
		idList = null;
		map = null;
