 ...
 * 湘西州部门办件统计明细
 */
public class DepartmentStatisticsSonExport {
	...
	public void excelExport(String titleName, List<Ta_Jc_Sum_Xxylttj> busList) {
		// 下层表头文字
		String[] coloumItems1 = { "所属乡镇","所属村","事项类型","事项名称", "受理数", "办结数","乡镇审核量","县级审核量"};
		// 下层表头坐标，顺序对应下层表头文字
		String[] number1 = { "1,1,1,1", "1,1,2,2", "1,1,3,3", "1,1,4,4","1,1,5,5","1,1,6,6","1,1,7,7","1,1,8,8"};
		// 开始写表格
	...
	private void writeExcel(String titleName, String[] coloumItems1,String[] number1, List<Ta_Jc_Sum_Xxylttj> busList) {
		...
		try {
			...
			sheet.setColumnWidth((short) 5, (short) 20 * 256);// 建议1
			sheet.setColumnWidth((short) 6, (short) 20 * 256);// 建议1
			sheet.setColumnWidth((short) 7, (short) 20 * 256);// 建议1
	...
	private void createValue(HSSFWorkbook workbook, HSSFSheet sheet,List<Ta_Jc_Sum_Xxylttj> busList) {
		...
		for (int j = 0; j < n_size; j++) {
			...
			cell.setCellStyle(styles);
			cell.setCellValue(obj.getRemarksName());
			
			// 建议1
			cell = row.createCell(2);
			cell.setCellStyle(styles);
			cell.setCellValue(obj.getApproveType());
									
									// 建议1
			cell = row.createCell(3);
			cell.setCellStyle(styles);
			cell.setCellValue(obj.getApproveName());
