...
public class HuanJieFaPaiStatisticsInfoExcelExport {
	...
	public void excelExport(String titleName, List<List<TongJi>> dataList) {
		...
		String[] coloumItems1 = { "行政区划/组织机构", "黄牌数(张)","红牌数(张)","撤销","撤销"};

		// 第二行，上层表头坐标，顺序对应上层表头文字，重复的表头只存一个
		String[] number1 = { "1,2,0,0", "1,2,1,1", "1,2,2,2","1,1,3,4"};
	...
	private void writeExcel(String titleName, String[] coloumItems1, String[] number1, String[] coloumItems2,
		...
		try {
			...
			sheet.setColumnWidth((short) 0, (short) 45 * 256);// 行政区划/组织机构
	...
	private void createValue(HSSFWorkbook workbook, HSSFSheet sheet, List<List<TongJi>> dataList) {
		...
		for (int i = 0; i < w_size; i++) {
			...
			for (int j = 0; j < n_size; j++) {
				...
				cell.setCellStyle(styles);
				cell.setCellValue(tongJi.getSuperviseSegmentYelloNum());
				//红牌数(张)
				cell = row.createCell(2);
				cell.setCellStyle(styles);
				cell.setCellValue(tongJi.getSuperviseSegmentRedNum());
				//撤销-黄牌数(张)
				cell = row.createCell(3);
				cell.setCellStyle(styles);
				cell.setCellValue(tongJi.getCancelSegmentYellowNum());
				//撤销-红牌数(张)
				cell = row.createCell(4);
				cell.setCellStyle(styles);
				cell.setCellValue(tongJi.getCancelSegmentRedNum());
