 ...
 *
 */
public class FenLeiShiXiangBanJianStatisticsInfoExcelExport {
	...
	private void createValue(HSSFWorkbook workbook, HSSFSheet sheet, List<List<TongJi>> dataList) {
		...
		for (int i = 0; i < w_size; i++) {
			...
			for (int j = 0; j < n_size; j++) {
				...
				// 实体对象
				TongJi shiXiangBanJianStatistics = dataList.get(i).get(j);
				...
				cell.setCellStyle(styles);
				cell.setCellValue(shiXiangBanJianStatistics.getAreaOrgName());
