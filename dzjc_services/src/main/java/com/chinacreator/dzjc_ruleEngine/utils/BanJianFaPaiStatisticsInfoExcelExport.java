...
public class BanJianFaPaiStatisticsInfoExcelExport {
	...
	private void createValue(HSSFWorkbook workbook, HSSFSheet sheet, List<List<TongJi>> dataList) {
		...
		for (int i = 0; i < w_size; i++) {
			...
			for (int j = 0; j < n_size; j++) {
				...
				cell.setCellStyle(styles);
				cell.setCellValue(tongJi.getInstanceSuperviseGreenNum());
				//黄牌数(张)
				cell = row.createCell(2);
				cell.setCellStyle(styles);
				cell.setCellValue(tongJi.getInstanceSuperviseYelloNum());
				//红牌数(张)
				cell = row.createCell(3);
				cell.setCellStyle(styles);
				cell.setCellValue(tongJi.getInstanceSuperviseRedNum());
				//撤销-黄牌数(张)
				cell = row.createCell(4);
				cell.setCellStyle(styles);
				cell.setCellValue(tongJi.getInstanceCancelYellowNum());
				//撤销-红牌数(张)
				cell = row.createCell(5);
				cell.setCellStyle(styles);
				cell.setCellValue(tongJi.getInstanceCancelRedNum());
