...
public class InstanceExport {
	...
	public void excelExport(String titleName,
			List<List<SuperviseInfoInstance>> dataList) {

		/** 创建相关参数 */

		// 下层表头文字
		String[] coloumItems1 = { "监管结果", "监管类型", "单位", "项目名称","申请者", "查询码", "受理时间",
				"结束时间","监管时间","办理状态","办件编号"};
	...
	private void writeExcel(String titleName, String[] coloumItems1,
			String[] number1, List<List<SuperviseInfoInstance>> dataList) {
		...
		try {
			...
			sheet.setColumnWidth((short) 10, (short) 40 * 256);// 办理编号
			// 设置第一行大标题
	...
	private void createValue(HSSFWorkbook workbook, HSSFSheet sheet,
		...
		for (int i = 0; i < w_size; i++) {
			...
			for (int j = 0; j < n_size; j++) {
				...
				// 实体对象
				SuperviseInfoInstance instance = dataList.get(i).get(j);
			

				// 监察结果
				cell = row.createCell(0);
				cell.setCellStyle(styles);
				cell.setCellValue(instance.getSuperviseResultName());
				...
				cell.setCellStyle(styles);
				cell.setCellValue(instance.getQueryCode());
				// 受理时间
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				cell = row.createCell(6);
				cell.setCellStyle(styles);

				if (instance.getAcceptTime() != null
						&& instance.getAcceptTime().toString() != "") {
				...
				if (instance.getEndTime() != null
						&& instance.getEndTime() != "") {
					cell.setCellValue(instance.getEndTime().substring(0, 10));
				}
