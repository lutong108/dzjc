...
public class HotApproveInfoExcelExport {
	 ...
	 * Excel导入准备工作
	 */
	public void excelExport(String titleName, List<List<HotApproveInfo>> dataList) {
		/** 创建相关参数 */

		// 第一行，上层表头文字，一个表头合并多少行，就重复写多少个
		String[] coloumItems1 = {"实施编码","业务办理项编码","事项名称","实施机关名称","事项类型","行使层级","收件数","办结数","红牌","黄牌","预警牌"};

		// 第二行，上层表头坐标，顺序对应上层表头文字，重复的表头只存一个
		String[] number1 = { "1,2,0,0", "1,2,1,1", "1,2,2,2","1,2,3,3","1,2,4,4","1,2,5,5","1,2,6,6","1,2,7,7","1,2,8,8","1,2,9,9","1,2,10,10"};
	...
	private void writeExcel(String titleName, String[] coloumItems1, String[] number1, String[] coloumItems2,
			String[] number2, List<List<HotApproveInfo>> dataList) {
	 ...
	 * @param s
	 */
	private void createValue(HSSFWorkbook workbook, HSSFSheet sheet, List<List<HotApproveInfo>> dataList) {
		...
		for (int i = 0; i < w_size; i++) {
			...
			for (int j = 0; j < n_size; j++) {
				...
				// 实体对象
				HotApproveInfo HotApproveInfo = dataList.get(i).get(j);
				...
				cell.setCellStyle(styles);
				cell.setCellValue(HotApproveInfo.getApproveCode());
				
				//业务办理项编码
				cell = row.createCell(1);
				cell.setCellStyle(styles);
				cell.setCellValue(HotApproveInfo.getTaskhandleitem());
				//事项名称","实施机关名称","事项类型","行使层级","收件数","办结数","红牌","黄牌","预警牌"
				cell = row.createCell(2);
				cell.setCellStyle(styles);
				cell.setCellValue(HotApproveInfo.getApproveName());
				//实施机关名称
				cell = row.createCell(3);
				cell.setCellStyle(styles);
				cell.setCellValue(HotApproveInfo.getOrgName());
				//事项类型
				cell = row.createCell(4);
				cell.setCellStyle(styles);
				String approveTypeTemp = HotApproveInfo.getTypeCode();
				String approveType = "";
				...
				cell.setCellStyle(styles);
				String exerciseHierarchyTemp = HotApproveInfo.getApproveSource();
				...
				cell.setCellStyle(styles);
				cell.setCellValue(HotApproveInfo.getSjsNum());
				//办结数
				cell = row.createCell(7);
				cell.setCellStyle(styles);
				cell.setCellValue(HotApproveInfo.getBjsNum());
				//红牌
				cell = row.createCell(8);
				cell.setCellStyle(styles);
				cell.setCellValue(HotApproveInfo.getRedNum());
				//黄牌
				cell = row.createCell(9);
				cell.setCellStyle(styles);
				cell.setCellValue(HotApproveInfo.getYellowNum());
				//预警牌
				cell = row.createCell(10);
				cell.setCellStyle(styles);
				cell.setCellValue(HotApproveInfo.getGreenNum());
