...
public class HotInstanceSuperviseExcelExport {
	 ...
	 * Excel导入准备工作
	 */
	public void excelExport(String titleName, List<List<HotInstanceSupervise>> dataList) {
		/** 创建相关参数 */

		// 第一行，上层表头文字，一个表头合并多少行，就重复写多少个
		String[] coloumItems1 = {"发牌结果名称","项目名称","单位","办理状态","受理时间","办结时间"};

		// 第二行，上层表头坐标，顺序对应上层表头文字，重复的表头只存一个
		String[] number1 = { "1,2,0,0", "1,2,1,1", "1,2,2,2","1,2,3,3","1,2,4,4","1,2,5,5"};
	...
	private void writeExcel(String titleName, String[] coloumItems1, String[] number1, String[] coloumItems2,
			String[] number2, List<List<HotInstanceSupervise>> dataList) {
	 ...
	 * @param s
	 */
	private void createValue(HSSFWorkbook workbook, HSSFSheet sheet, List<List<HotInstanceSupervise>> dataList) {
		...
		for (int i = 0; i < w_size; i++) {
			...
			for (int j = 0; j < n_size; j++) {
				...
				// 实体对象
				HotInstanceSupervise HotInstanceSupervise = dataList.get(i).get(j);
				...
				cell.setCellStyle(styles);
				cell.setCellValue(HotInstanceSupervise.getSuperviseResultName());
				
				//办件名称
				cell = row.createCell(1);
				cell.setCellStyle(styles);
				cell.setCellValue(HotInstanceSupervise.getInstanceName());
				
				//单位
				cell = row.createCell(2);
				cell.setCellStyle(styles);
				cell.setCellValue(HotInstanceSupervise.getOrgName());
				
				//办理状态
				cell = row.createCell(3);
				cell.setCellStyle(styles);
				cell.setCellValue(HotInstanceSupervise.getBjState());
				
				//受理时间
				cell = row.createCell(4);
				cell.setCellStyle(styles);
				Date acceptTime = HotInstanceSupervise.getAcceptTime();
				String acceptTimeStr = "";
				if(acceptTime != null){
					acceptTimeStr = sf.format(acceptTime);
				}
				cell.setCellValue(acceptTimeStr);
				
				//办结时间"
				cell = row.createCell(5);
				cell.setCellStyle(styles);
				Date endTime = HotInstanceSupervise.getEndTime();
				String endTimeStr = "";
				if(endTime != null){
					endTimeStr = sf.format(endTime);
				}
