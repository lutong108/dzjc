...
(function() {
	...
	var tF = function(cC) {
		return (cC.global ? "g" : "") + (cC.ignoreCase ? "i" : "")
				+ (cC.multiline ? "m" : "") + (cC.extended ? "x" : "")
				+ (cC.sticky ? "y" : "");
	};
	var mZ = function(o) {
	...
	RegExp.prototype.exec = function(bM) {
		var match = eS.exec.apply(this, arguments), name, r2;
		if (match) {
			if (!rJ && match.length > 1 && uF(match, "") > -1) {
				r2 = RegExp("^" + this.source + "$(?!\\s)", tF(this));
				eS.replace.call(match[0], r2, function() {
	...
	String.prototype.split = function(s, dq) {
		...
		var bM = this + "", output = [], hz = 0, match, mp;
...
var F = (function() {
	...
	return {
		...
		AM : s.fw('chrome/'),
		xb : s.fw(' applewebkit/'),
		Cq : window.navigator.platform == "Win64",
...
var am = (function() {
	...
	var mv = function() {
		...
		kP = "<html><head>" + "<link href='" + EWEB.cd + "/skin/" + config.Skin
				+ "/menuarea.css' type='text/css' rel='stylesheet'>"
				+ "</head>" + "<body scroll='no'>";
		if (F.as) {
	...
	var dY = function(aW, pP, jn, V) {
		...
		if (F.as) {
			...
			js = "var w=frameElement.kw;w." + pP + ";w.am.Hide();"
		}
		if (aW == "") {
			dX += "<tr><td class='Menu_Item'><table border=0 cellpadding=0 cellspacing=0 width='100%'><tr><td valign=middle class=MouseOut onMouseOver=\"this.className='MouseOver'\" onMouseOut=\"this.className='MouseOut'\" onclick=\""
					+ js + "\">";
		} else {
		...
		if (typeof (jn) == "number") {
			...
			var dM = 16 - jn * 16;
			dX += "<div class='Menu_Image" + aW + "'><img src='" + cA
					+ "' style='top:" + dM + "px'></div>";
		} else if (jn != "") {
			var cA = "skin/" + config.Skin + "/" + jn;
			dX += "<img class='Menu_Image" + aW + "' src='" + cA + "'>";
		}
		dX += "</td><td class='Menu_Label" + aW + "'>" + V
				+ "</td></tr></table>";
		...
		if (C.ai() == "Control") {
			if (aH) {
				...
				if (el.tagName.toUpperCase() == aH) {
					if ((dg) && (iG)) {
						if (el.getAttribute(dg, 2).toLowerCase() == iG
								.toLowerCase()) {
							return true;
						}
					} else {
	...
	return {
		...
		fJ : function(e, an) {
			...
			case "table":
				...
				break;
			case "form":
				G += bb("FormText", "InsertInputText");
				G += bb("FormTextArea", "InsertTextArea");
				G += bb("FormRadio", "InsertInputRadio");
				G += bb("FormCheckbox", "InsertInputCheckbox");
				G += bb("FormDropdown", "InsertSelectDropdown");
				G += bb("FormButton", "InsertButton");
				break;
			case "gallery":
				G += O("", "GalleryImage");
				G += O("", "GalleryFlash");
			...
			case "zoom":
				for ( var i = 0; i < cY.Options.length; i++) {
					if (cY.Options[i] == cY.qr) {
						G += dY("", "cY.Execute(" + cY.Options[i] + ")", 120,
								cY.Options[i] + "%");
					} else {
						G += dY("", "cY.Execute(" + cY.Options[i] + ")", 119,
								cY.Options[i] + "%");
					}
				}
				break;
			case "fontsize":
				...
				for ( var i = 0; i < lang["FontSizeItem"].length; i++) {
					if (lang["FontSizeItem"][i][0] == v) {
						G += dY("", "fq('size','" + lang["FontSizeItem"][i][0]
								+ "')", 120, lang["FontSizeItem"][i][1]);
					} else {
						G += dY("", "fq('size','" + lang["FontSizeItem"][i][0]
								+ "')", 119, lang["FontSizeItem"][i][1]);
					}
				}
				break;
			case "fontname":
				...
				for ( var i = 0; i < lang["FontNameItem"].length; i++) {
					if (lang["FontNameItem"][i] == v) {
						G += dY("", "fq('face','" + lang["FontNameItem"][i]
								+ "')", 120, lang["FontNameItem"][i]);
					} else {
						G += dY("", "fq('face','" + lang["FontNameItem"][i]
								+ "')", 119, lang["FontNameItem"][i]);
					}
				}
				break;
			case "formatblock":
				...
				for ( var i = 0; i < lang["FormatBlockItem"].length; i++) {
					if (lang["FormatBlockItem"][i][0].toLowerCase() == v) {
						G += dY("", "format('FormatBlock','"
								+ lang["FormatBlockItem"][i][0] + "')", 120,
								lang["FormatBlockItem"][i][1]);
					} else {
						G += dY("", "format('FormatBlock','"
								+ lang["FormatBlockItem"][i][0] + "')", 119,
								lang["FormatBlockItem"][i][1]);
					}
				}
				break;
			...
			if (cv) {
				...
				bH.y = y;
				bH.ew = ew;
				bH.rel = R.fR(document) ? document.documentElement
						: document.body;
				am.Show();
			} else {
				var el = e.target;
				if (el.tagName.toLowerCase() == "img"
						|| el.className == "TB_Btn_Image") {
					el = el.parentNode;
				}
				var x = 0;
				var y = 0;
...
var ec = (function() {
	...
	var oH = function() {
		...
		R.cP(cB, {
			'width' : Math.max(Bk.scrollWidth, Bk.clientWidth,
					cg.scrollWidth || 0)
					- 1 + 'px',
			'height' : Math.max(Bk.scrollHeight, Bk.clientHeight,
					cg.scrollHeight || 0)
					- 1 + 'px'
		});
	};
...
var ci = (function() {
	...
	var kT = function() {
		...
		var oa = {
			...
			other : [ "ShowBlocks", "ShowBorders", "Maximize", "FormatBrush",
					"ExpandToolbar" ]
		};
	...
	var uV = function(ab, aF) {
		...
		if (ab.eg("pt") && aF.eg("px")) {
			...
			if (gj > 0) {
				if (ab == Math.round(parseFloat(aF) * 3 / 4 * Math.pow(10, gj)
						+ 0.01)
						/ Math.pow(10, gj)) {
					return true;
				}
	...
	return {
			...
			if (C.ai() != "Control") {
				...
				while (el) {
					...
					if (el.tagName.toUpperCase() == "TD"
							|| el.tagName.toUpperCase() == "TH") {
						break;
					} else if (el.tagName.toUpperCase() == "BODY"
		...
		qt : function() {
			...
			for ( var i = 0; i < cz.length; i++) {
				cQ = cQ + cz[i].getAttribute('colSpan');
			}
			var gu = dk.insertRow(au.rowIndex);
			for (i = 0; i < cQ; i++) {
				fL = gu.insertCell(-1);
				fL.innerHTML = "&nbsp;";
			}
		...
		rv : function() {
			...
			var ze = aS.getAttribute('rowSpan');
			ba = dk.rows;
			if (au.rowIndex + 1 != ba.length) {
				var rq = ba[au.rowIndex + aS.rowSpan].cells;
				var lx = rq[aS.cellIndex].getAttribute('rowSpan');
		...
		pI : function(mu) {
			...
			while (aS.rowSpan > 1 && jQ > 0) {
				...
				for ( var n = 0; n < kM.cells.length; n++) {
					gJ += kM.cells[n].getAttribute('colSpan');
					if (gJ > gp) {
						position = n;
						break;
					}
			...
			for ( var n = 0; n < jQ; n++) {
				...
				for ( var j = 0; j < au.rowIndex; j++) {
					for ( var k = 0; k < ba[j].cells.length; k++) {
						if ((ba[j].cells[k].rowSpan > 1)
								&& (ba[j].cells[k].rowSpan >= au.rowIndex
										- ba[j].rowIndex + 1)) {
		...
		sq : function() {
			...
			var sH = aS.getAttribute('colSpan');
			cz = au.cells;
			if (aS.cellIndex + 1 != au.cells.length) {
				var ru = cz[aS.cellIndex + 1].getAttribute('colSpan');
				aS.colSpan = sH + ru;
		...
		qY : function(mJ) {
			...
			for (i = 0; i < ba.length; i++) {
				...
				for ( var n = 0; n < ba[i].cells.length; n++) {
					gJ += ba[i].cells[n].getAttribute('colSpan');
					if (gJ + lb > gp) {
						position = n;
						break;
					}
...
var cW = (function() {
	...
	return {
		...
		MM : function(e) {
			...
			while (el.tagName.toUpperCase() != "TD"
					&& el.tagName.toUpperCase() != "TH") {
			...
			if (df || dQ) {
				kj = mf + $("eWebEditor").offsetLeft
						+ $("eWebEditor_Layout").offsetLeft;
				nF = $("eWebEditor").offsetTop
						+ $("eWebEditor_Layout").offsetTop
						+ $("eWebEditor_ToolarPTR").offsetHeight;
				dz.style.left = kj + "px";
				dz.style.top = nF + "px";
				dz.style.width = or + "px";
				dz.style.height = $("eWebEditor").offsetHeight + "px";
				nC = $("eWebEditor").offsetLeft
						+ $("eWebEditor_Layout").offsetLeft;
				lB = mo + $("eWebEditor").offsetTop
						+ $("eWebEditor_Layout").offsetTop
						+ $("eWebEditor_ToolarPTR").offsetHeight;
				cU.style.left = nC + "px";
				cU.style.top = lB + "px";
		...
		MD2 : function(e) {
			...
			if (df) {
				...
				for ( var i = 0; i < av.rows.length; i++) {
					for ( var j = 0; j < av.rows[i].cells.length; j++) {
						var be = av.rows[i].cells[j];
						if ((be.offsetLeft + be.offsetWidth) >= (cJ.offsetLeft + cJ.offsetWidth)
								&& (be.offsetLeft < (cJ.offsetLeft + cJ.offsetWidth))) {
							hc[n] = be;
	...
	return {
		...
		Save : function() {
			...
			if (F.as) {
				if (C.ai() != "Control") {
					try {
						dR[dR.length] = EWEB.T.selection.createRange()
								.getBookmark();
					} catch (e) {
						dR[dR.length] = "";
					}
...
var EWEB = {
			...
			if (config.FixWidth) {
				...
				try {
					...
					if (aD.parentElement().tagName == "BODY") {
						aD.moveToElementText(EWEB.T
								.getElementById("eWebEditor_FixWidth_DIV"));
						aD.collapse(true);
						aD.select();
					} else {
						aD.select();
	...
	Init : function() {
		...
		this.hW = document.location.protocol.toLowerCase() + "//"
		...
		if (aA.ExtCSS) {
			this.ExtCSS = "<link href='" + uN(aA.ExtCSS)
		...
		this.SendUrl = this.hW + this.cd + "/" + config.ServerExt + "/upload."
				+ config.ServerExt + "?style=" + aA.StyleName + "&cusdir="
				+ aA.CusDir + "&sparams=" + config.SParams + "&skey=" + aA.SKey
				+ "&h=" + aA.H;
...
function Dj() {
	...
	if (ifr_Editarea) {
		var be = ifr_Editarea.parentNode;
		var Ee = parseInt(R.ce(ifr_Editarea, "border-top-width"));
		var EF = parseInt(R.ce(ifr_Editarea, "border-bottom-width"));
		var Dv = parseInt(R.ce(ifr_Editarea, "border-left-width"));
		var EG = parseInt(R.ce(ifr_Editarea, "border-right-width"));
		ifr_Editarea.style.width = (be.clientWidth - Dv - EG) + 'px';
		ifr_Editarea.style.height = (be.clientHeight - Ee - EF) + 'px';
		ifr_Editarea.style.height = '100%';
		DN();
	} else {
...
function DN() {
	...
	if (config.FixWidth) {
		if (F.Ei) {
			var J = ifr_Editarea.contentWindow.document;
			var dK = J.body;
			var cR = J.getElementById("eWebEditor_FixWidth_DIV");
			if (!cR) {
				return;
			}
			var DI = parseInt(R.ce(dK, "padding-top"));
			var DE = parseInt(R.ce(dK, "padding-bottom"));
			var DA = parseInt(R.ce(cR, "border-top-width"));
			var EL = parseInt(R.ce(cR, "border-bottom-width"));
			var En = parseInt(R.ce(cR, "padding-top"));
			var EC = parseInt(R.ce(cR, "padding-bottom"));
			cR.style.minHeight = (ifr_Editarea.clientHeight - DI - DE - DA - EL
					- En - EC)
					+ "px";
		}
	} else {
		if (F.Ei && F.as) {
			var dK = ifr_Editarea.contentWindow.document.body;
			var DI = parseInt(R.ce(dK, "padding-top"));
			var DE = parseInt(R.ce(dK, "padding-bottom"));
			dK.style.minHeight = (ifr_Editarea.clientHeight - DI - DE) + "px";
		}
	}
};
...
function ts() {
	...
	if (!EWEB.bs) {
		alert('[EWEBEDITOR] The element with id or name "' + aA.bs
				+ '" was not found.');
		return;
	}
	if (EWEB.bs.tagName == "TEXTAREA"
			|| EWEB.bs.tagName.substring(0, 5) == "INPUT") {
		EWEB.hq = "INPUT";
	} else {
		EWEB.hq = "OTHER";
	}
...
function sU(e) {
	...
	if (e.ctrlKey || ((bK >= 33) && (bK <= 40)) || (bK == 13) || (bK == 8)
			|| (bK == 46)) {
		ci.eW();
	}
	return true;
...
function sZ(e) {
	...
	if (!bo && C.ai() == "Text") {
		if (!(e.ctrlKey && bA != "D" && bA != "V") && !e.metaKey && !e.shiftKey
				&& !e.altKey) {
			var Ep = C.cI();
...
function insertHTML(V) {
	...
	case "EDIT":
		...
		if (!window.getSelection) {
			...
			V = '<span id="__ewebeditor_temp_remove__" style="display:none;">eWebEditor</span>'
					+ V;
			EWEB.T.selection.createRange().pasteHTML(V);
			EWEB.T.getElementById('__ewebeditor_temp_remove__')
					.removeNode(true);
		} else {
	...
	case "CODE":
		...
		if (document.selection) {
			...
			var ED = wa.scrollTop;
			wa.value = wa.value.substring(0, CX) + V
					+ wa.value.substring(Ev, wa.value.length);
			if (ED > 0) {
				wa.scrollTop = ED;
			}
...
function setHTML(V, ub) {
	...
	case "TEXT":
		...
		$("eWebEditor_Temp_HTML").innerHTML = V;
		if (F.as) {
			V = $("eWebEditor_Temp_HTML").innerText;
		} else {
			V = kZ($("eWebEditor_Temp_HTML"));
		}
		EWEB.db.value = V;
		break;
	case "EDIT":
		...
		if (F.as) {
			if (config.FixWidth) {
				R.az(EWEB.T.getElementById("eWebEditor_FixWidth_DIV"),
						'beforedeactivate', function() {
							C.Save();
						});
			} else {
				R.az(EWEB.T, 'beforedeactivate', function() {
					C.Save();
				});
			}
		}
...
function openUploadDialog(v, dF, pw, pN, qK, hA) {
	...
	var aG = 'i_upload.htm?type=' + cT + '&mode=' + dF + '&savepathfilename='
			+ pw + '&savefilename=' + pN + '&originalfilename=' + qK;
	if (hA) {
		aG += '&returnflag=' + hA;
	}
	ec.OpenDialog(aG);
...
function kZ(el) {
	var sl = [ "p", "div", "h1", "h2", "h3", "h4", "h5", "h6", "pre", "ol",
			"ul", "pre", "fieldset", "form", "table", "blockquote", "dl", "li",
			"br" ];
	var rW = [ "script", "style", "table", "object", "embed" ];
	var bE = "";
...
function pb(hv, V) {
	...
	var mU = R.dV(hv);
	var ga = 50000;
	var K = mU.getElementsByName(ny);
	for ( var i = 1; i < K.length; i++) {
		K[i].value = "";
	}
	if (V.length > ga) {
		hv.value = V.substr(0, ga);
		V = V.substr(ga);
		while (V.length > 0) {
			var fV;
			if (F.as) {
				fV = mU
						.createElement('<textarea name="' + ny
								+ '"></textarea>');
			} else {
				fV = mU.createElement("textarea");
				fV.setAttribute("name", ny);
			}
			fV.style.display = "none";
			fV.value = V.substr(0, ga);
			aO.appendChild(fV);
			V = V.substr(ga);
		}
	}
...
function localUpload() {
	...
	nz(lang["MsgLocalUploading"]);
	var fh = getHTML();
	aZ.LocalUpload(fh);
	window.setTimeout("od()", 100);
};
function od() {
	if (aZ.Status != "ok") {
		window.setTimeout("od()", 100);
	...
	var ek = aZ.OriginalFiles;
	var gw = aZ.SavedFiles;
	if (ek) {
		var ea = ek.split("|");
		var ey = gw.split("|");
...
function pG(V) {
	...
	dS += '<link href="' + EWEB.cd + '/skin/' + config.Skin
			+ '/editarea.css" type="text/css" rel="stylesheet">';
	switch (EWEB.ae) {
	case 'CODE':
		break;
	case 'TEXT':
	...
	case 'VIEW':
		if (config.AreaCssMode != '1') {
			dS += '<link href="' + EWEB.cd + '/skin/' + config.Skin
					+ '/editarea.zh-cn.css" type="text/css" rel="stylesheet">';
		}
		dS += EWEB.ExtCSS;
		break;
	}
	...
	if (config.FixWidth) {
		aI += ' class="ewebeditor__fixwidth">'
				+ '<div id="eWebEditor_FixWidth_DIV" style="width:'
				+ config.FixWidth + ';" >';
...
function getText() {
	...
	if (EWEB.ae == "TEXT") {
		...
		if (F.as) {
			bE = $("eWebEditor_Temp_HTML").innerText;
		} else {
			bE = kZ($("eWebEditor_Temp_HTML"));
		}
	}
	return bE;
};
function ng(url) {
	document.write('<scr' + 'ipt type="text/javascript" src="' + url
			+ '" onerror="alert(\'Error loading \' + this.src);"><\/scr'
			+ 'ipt>');
};
function wr() {
	var bC = "";
	...
	bC += '<iframe id="eWebEditor" style="width:100%;height:100%"  frameborder="0" allowTransparency="true" src="'
			+ R.iO() + '"></iframe>';
	bC += "</td></tr>";
	if (config.StateFlag) {
		...
		if (config.SBCode) {
			bC += "		<td class=SB_Mode_BtnOff id=eWebEditor_CODE onclick=\"setMode('CODE')\" unselectable=on><table border=0 cellpadding=0 cellspacing=0><tr><td class=SB_Mode_Btn_Img>"
					+ ef("ModeCode")
					+ "</td><td class=SB_Mode_Btn_Text>"
					+ lang["StatusModeCode"] + "</td></tr></table></td>";
			bC += "		<td class=SB_Mode_Sep></td>";
		}
		if (config.SBEdit) {
			bC += "		<td class=SB_Mode_BtnOff id=eWebEditor_EDIT onclick=\"setMode('EDIT')\" unselectable=on><table border=0 cellpadding=0 cellspacing=0><tr><td class=SB_Mode_Btn_Img>"
					+ ef("ModeEdit")
					+ "</td><td class=SB_Mode_Btn_Text>"
					+ lang["StatusModeEdit"] + "</td></tr></table></td>";
			bC += "		<td class=SB_Mode_Sep></td>";
		}
		if (config.SBText) {
			bC += "		<td class=SB_Mode_BtnOff id=eWebEditor_TEXT onclick=\"setMode('TEXT')\" unselectable=on><table border=0 cellpadding=0 cellspacing=0><tr><td class=SB_Mode_Btn_Img>"
					+ ef("ModeText")
					+ "</td><td class=SB_Mode_Btn_Text>"
					+ lang["StatusModeText"] + "</td></tr></table></td>";
			bC += "		<td class=SB_Mode_Sep></td>";
		}
		if (config.SBView) {
			bC += "		<td class=SB_Mode_BtnOff id=eWebEditor_VIEW onclick=\"setMode('VIEW')\" unselectable=on><table border=0 cellpadding=0 cellspacing=0><tr><td class=SB_Mode_Btn_Img>"
					+ ef("ModeView")
					+ "</td><td class=SB_Mode_Btn_Text>"
					+ lang["StatusModeView"] + "</td></tr></table></td>";
		}
		bC += "		</tr>";
		bC += "		</table>";
		bC += "	</td>";
		if (aA.FullScreen != "1" && config.SBSize) {
			...
			bC += "		<tr>";
			bC += "		<td class=SB_Size_Btn onclick='ej(300)' title='"
					+ lang["SizePlus"] + "'>" + ef("SizePlus") + "</td>";
			bC += "		<td class=SB_Size_Sep></td>";
			bC += "		<td class=SB_Size_Btn onclick='ej(-300)' title='"
					+ lang["SizeMinus"] + "'>" + ef("SizeMinus") + "</td>";
	...
	bC += "<form id='eWebEditor_UploadForm' action='" + config.ServerExt
			+ "/upload." + config.ServerExt
			+ "?action=remote&type=remote&style=" + aA.StyleName
			+ "&language=zh-cn&cusdir=" + aA.CusDir + "&skey=" + aA.SKey
			+ "' method='post' target='eWebEditor_UploadTarget'>";
	bC += "<input type='hidden' name='eWebEditor_UploadText'>";
	bC += "</form>";
	bC += '<iframe name="eWebEditor_UploadTarget" width=0 height=0 src="'
			+ R.iO() + '"></iframe>';
	bC += "</div>";
	bC += "<div id=divProcessing style='width:200px;height:30px;position:absolute;display:none'>";
...
function qy(qH, sB) {
	var Cw = false;
	var aI = "<table border='0' cellpadding='0' cellspacing='0' width='100%' id='eWebEditor_Toolbar_"
			+ sB + "' unselectable>";
	for ( var i = 0; i < qH.length; i++) {
		aI += "<tr id='eWebEditor_Toolbar_"
				+ sB
				+ "_TR"
				+ i
				+ "' style='display:"
				+ (Cw ? "none" : "")
				+ "'><td class='TB_Left'></td><td class='TB_Center'><table border='0' cellpadding='0' cellspacing='0'><tr>";
		var tb = qH[i];
		for ( var j = 0; j < tb.length; j++) {
			...
			if (aM == "TBSep") {
				...
				aI += "<td class='TB_Btn_Padding'><div class='TB_Btn' name='TB_Name_"
						+ aM
						+ "' title=\""
						+ lang[aM]
						+ "\" onclick=\""
				...
				if (typeof (di[0]) == "number") {
					...
					var dM = 16 - di[0] * 16;
					if (F.as) {
						aI += "<div class='TB_Btn_Image'><img src='" + cA
								+ "' style='top:" + dM + "px' /></div>";
					} else {
						aI += "<img class='TB_Btn_Image' src='sysimage/space.gif' style='background-position: 0px "
								+ dM
				...
				switch (aM) {
				case "FontName":
					fS = " style='width:115px'";
					for ( var k = 0; k < lang[aM + "Item"].length; k++) {
						bW += "<option value='" + lang[aM + "Item"][k] + "'>"
								+ lang[aM + "Item"][k] + "</option>";
					}
					break;
				case "FontSize":
					fS = " style='width:55px'";
					for ( var k = 0; k < lang[aM + "Item"].length; k++) {
						bW += "<option value='" + lang[aM + "Item"][k][0]
								+ "'>" + lang[aM + "Item"][k][1] + "</option>";
					}
					break;
				case "FormatBlock":
					fS = " style='width:90px'";
				...
				case "ZoomSelect":
					...
					for ( var k = 0; k < cY.Options.length; k++) {
						bW += "<option value='" + cY.Options[k] + "'>"
								+ cY.Options[k] + "%</option>";
					}
					break;
				}
				aI += "<td class='TB_Btn_Padding'><select name='TB_Name_" + aM
						+ "' onchange=\"" + hL + "\" size=1 " + fS
						+ "><option selected>" + lang[aM] + "</option>" + bW
						+ "</select></td>";
			}
		}
...
function ef(aM, rg) {
	...
	if (F.as) {
		...
		return "<img class='SB_Btn_Image' src='sysimage/space.gif' style='background-position: 0px "
				+ dM + "px;background-image: url(" + cA + ");' />";
	}
};
var dp = new Object();
dp.fv = function() {
...
aE.vb = function(V) {
	function _Replace(m, aH) {
		function gm(m, bv) {
			if ([ 'flash', 'flv', 'mediaplayer6', 'mediaplayer7', 'realplayer',
					'quicktime', 'unknownobject' ].IndexOf(aH) >= 0) {
				var ak = aE.pr(m, 'img');
				if (ak != '') {
					ak = ' style=' + ak;
				}
				var eh = (aE.mA(m, 'img', 'width') == '') ? aE.co(m, 'img',
						'width') : '';
				var ff = (aE.mA(m, 'img', 'height') == '') ? aE.co(m, 'img',
						'height') : '';
				...
				eu = aE.sv(eu, [ "object", "embed" ], [ [ "width", eh ],
						[ "height", ff ], [ "align", fQ ], [ "vspace", jB ],
						[ "hspace", gq ] ]);
				return eu;
			} else {
		...
		return m
				.replace(
						/<img [^>]*?_ewebeditor_fake_value=\"([^\">]+?)\"[^>]*?>/gi,
						gm);
	}
	;
	return V.replace(/<img [^>]*?_ewebeditor_fake_tag=\"(\w+?)\"[^>]*?>/gi,
			_Replace);
};
aE.rZ = function(V) {
	if (config.PaginationMode == "0") {
	...
	if (config.PaginationMode == "1") {
		...
		var re = new RegExp(config.PaginationKey.replace(
				/([\[\]\{\}\.\(\)\*\+\?])/gi, "\\$1"), 'gi');
		aI = V.replace(re, cA);
	}
...
aE.sI = function(V) {
	function _Replace(m) {
		...
		} else if (dG == '0002E510-0000-0000-C000-000000000046'
				|| dG == '0002E551-0000-0000-C000-000000000046'
				|| dG == '0002E559-0000-0000-C000-000000000046') {
			return m;
		} else {
...
aE.uU = function(V) {
	function _Replace(m) {
		var cT = m
				.replace(
						/<embed [^>]*?type\s*=\s*[\'\"]?([^\'\"\s]+)[\'\"]?[^>]*?>[\s\S]*/gi,
						'$1');
		cT = cT.toLowerCase();
		if (cT == 'application/x-shockwave-flash') {
			return aE.ct('flash', m, 'embed');
		} else if ([ 'application/x-mplayer2', 'video/x-ms-asf',
				'video/x-msvideo', 'video/mpeg', 'audio/mid', 'audio/mpeg',
				'audio/wav', 'video/x-ms-wm', 'audio/x-ms-wma',
				'video/x-ms-wmv', 'video/x-ms-wmp', 'video/x-ms-wmx' ]
				.IndexOf(cT) >= 0) {
			return aE.ct('mediaplayer6', m, 'embed');
		} else if (cT == 'video/quicktime') {
			return aE.ct('quicktime', m, 'embed');
		} else if ([ 'audio/x-pn-realaudio', 'audio/x-pn-realaudio-plugin',
				'application/vnd.rn-realmedia' ].IndexOf(cT) >= 0) {
...
aE.ct = function(aH, bv, eJ) {
	if (eJ) {
		...
		var V = '<img src="' + EWEB.cd
				+ '/sysimage/space.gif" class="ewebeditor__' + aH
				+ '" _ewebeditor_fake_tag="' + aH
				+ '" _ewebeditor_fake_value="' + encodeURIComponent(bv) + '"'
				+ ak;
		if (eh != '') {
			V += ' width="' + eh + '"';
		}
...
aE.co = function(V, aH, bL) {
	var re = new RegExp('^[\\s\\S]*?<' + aH + '(?=[\\s>])[^>]*?\\s' + bL
			+ '\\s*?=\\s*?([\'\"]?)(\\w+)\\1[^>]*?>[\\s\\S]*$', 'gi');
	if (re.test(V)) {
		return V.replace(re, '$2');
	} else {
...
aE.sX = function(V) {
	function _Replace(m) {
		function gC(m, bL) {
			return ' _ewebeditor_pa_' + bL + '="' + encodeURIComponent(m) + '"';
		}
		;
...
aE.sQ = function(V) {
	...
	;
	return V.replace(/\s_ewebeditor_pa_\w+=\"([^\"]+)\"/gi, _Replace);
};
aE.pa = function(V, aH, bL) {
	function _Replace(m, m1, m2, m3, m4, m5) {
		if (/_ewebeditor_ta_/.test(m) || /_ewebeditor_fake_/.test(m)) {
			return m;
		} else {
			return m1 + m2 + ' _ewebeditor_ta_' + bL + '="'
					+ encodeURIComponent(m4.replace(/\"/gi, "'")) + '"' + m5;
		}
	}
	;
	var re = new RegExp('(<' + aH + '(?=\\s)[^>]*?)(\\s' + bL
			+ '\\s*?=\\s*?([\'\"])([^>]*?)\\3)([\\s\\S]*?>)', 'gi');
	return V.replace(re, _Replace);
};
aE.sO = function(V) {
	function _Replace(m, bL, bv) {
		...
		var s = m.replace(r, '');
		r = new RegExp('\\s_ewebeditor_ta_' + bL + '+\\s*?=\\s*?\"[^\"]*?\"',
				'gi');
		return s.replace(r, ' ' + bL + '="' + decodeURIComponent(bv) + '"');
	}
	;
	return V.replace(
			/<\w+[^>]*?_ewebeditor_ta_(\w+)\s*?=\s*?\"([^\">]*?)\"[^>]*>/gi,
			_Replace);
};
aE.sG = function(V) {
	V = V.replace(
			/(<\w+(?=\s)[^>]*?\sclass\s*?=[^>]*?)(ewebeditor__\w+)([^>]*?>)/gi,
			'$1$3');
	V = V.replace(/(<\w+(?=\s)[^>]*?)(\sclass\s*?=\s*?\"\s*\")([^>]*>)/gi,
			'$1$3');
	return V;
};
...
var vy = {
	vi : function(V) {
		...
		V = V
				.replace(
						/\<(P|DIV|H1|H2|H3|H4|H5|H6|ADDRESS|PRE|OL|UL|LI|DL|DT|DD|TITLE|META|LINK|BASE|SCRIPT|LINK|TD|TH|AREA|OPTION)[^\>]*\>/gi,
						'\n$&');
		V = V
				.replace(
						/\<\/(P|DIV|H1|H2|H3|H4|H5|H6|ADDRESS|PRE|OL|UL|LI|DL|DT|DD|TITLE|META|LINK|BASE|SCRIPT|LINK|TD|TH|AREA|OPTION)[^\>]*\>/gi,
	...
	for ( var i = 0; i < parent.frames.length; i++) {
		try {
			if (parent.frames[i].document == self.document) {
				var bQ = parent.frames[i].frameElement;
				var cq = parseInt(bQ.offsetHeight);
				if (cq + lW >= config.UIMinHeight) {
					bQ.height = cq + lW;
				} else {
					bQ.height = config.UIMinHeight;
				}
				break;
			}
		} catch (e) {
...
function qG() {
	...
	cR.innerHTML = '<span style="margin:0;padding:0;border:0;clear:both;width:1px;height:1px;display:block;">'
			+ (F.xb ? '&nbsp;' : '') + '</span>';
...
function iY() {
	...
	if (F.as) {
		var fH = EWEB.T.selection.createRange();
		var dK = EWEB.T.body;
		var hF = dK.getElementsByTagName("P");
		var eo = dK.createTextRange();
		for ( var i = 0; i < hF.length; i++) {
			eo.moveToElementText(hF[i]);
			if (fH.inRange(eo)) {
				return true;
			} else {
				if (((fH.compareEndPoints("StartToEnd", eo) < 0) && (fH
						.compareEndPoints("StartToStart", eo) > 0))
						|| ((fH.compareEndPoints("EndToStart", eo) > 0) && (fH
								.compareEndPoints("EndToEnd", eo) < 0))) {
					return true;
				}
			}
		}
	} else {
		var fH = EWEB.aR.getSelection();
		var dK = EWEB.T.body;
		var hF = dK.getElementsByTagName("P");
		for ( var i = 0; i < hF.length; i++) {
			if (fH.containsNode(hF[i], true)) {
				return true;
			}
		}
	}
...
function dB(aN) {
	...
	var bE;
	if (F.as) {
		var bk = EWEB.T.selection.createRange();
		bE = bk.text;
	} else {
		bE = EWEB.aR.getSelection().toString();
	}
	switch (aN) {
	case "nowdate":
		var d = new Date();
	...
	case "br":
		...
		break;
	case "code":
		insertHTML('<table width=95% border="0" align="Center" cellpadding="6" cellspacing="0" style="border: 1px Dotted #CCCCCC; TABLE-LAYOUT: fixed"><tr><td bgcolor=#FDFDDF style="WORD-WRAP: break-word"><font style="color: #990000;font-weight:bold">'
				+ lang["HtmlCode"]
				+ '</font><br>'
				+ fn(bE)
				+ '</td></tr></table>');
		break;
	case "quote":
		insertHTML('<table width=95% border="0" align="Center" cellpadding="6" cellspacing="0" style="border: 1px Dotted #CCCCCC; TABLE-LAYOUT: fixed"><tr><td bgcolor=#F3F3F3 style="WORD-WRAP: break-word"><font style="color: #990000;font-weight:bold">'
				+ lang["HtmlQuote"]
				+ '</font><br>'
				+ fn(bE)
				+ '</td></tr></table>');
		...
		for ( var i = 0; i < by.length; i++) {
			...
			if ((bd.tagName == "FONT") || (bd.tagName == "SPAN")) {
				if (bd.innerText == dI.innerText) {
					bd.style.cssText = bd.style.cssText + ";"
							+ dI.style.cssText;
...
function kJ(obj) {
	...
	for ( var j = 0; j < aY.length; j++) {
		...
		if ((aY[j].tagName == "FONT") || (aY[j].tagName == "SPAN")) {
			if ((aY[j].style.cssText == "" && aY[j].className == "")
					|| (aY[j].innerHTML == "")) {
...
var nL = (function() {
	...
	return {
		og : function(aV, fN) {
			...
			cO = aD.endOffset;
			if (aB == fp) {
				if (aB.nodeType == 1 && aB.tagName == "BODY") {
					jT = true;
					fp = this.me(aB);
					aB = this.oZ(aB);
					if (!aB || !fp) {
					...
					this.jG(aD.startContainer);
				} else {
					jU = true;
					var fm = aB.nodeValue;
					if (ft) {
						aB.nodeValue = fm.substring(0, cL)
								+ this.jh(fm.substring(cL, cO))
								+ fm.substring(cO);
					} else {
						...
						if (ao.tagName.toUpperCase() == "SPAN"
								&& ao.innerHTML == oi) {
							this.hx(ao);
						} else {
							var aU = this.ic(oi);
							aU.setAttribute("id",
									"eWebEditor_Temp_Span_FontSize", 0);
			...
			if (jT) {
				EWEB.T.execCommand("SelectAll", false, null);
			} else {
				if (ft) {
					aD.setStart(aB, cL);
					aD.setEnd(fp, cO);
					...
					if (jU) {
						var ib = EWEB.T
								.getElementById("eWebEditor_Temp_Span_FontSize");
						if (ib) {
							ib.removeAttribute("id");
							aD.selectNodeContents(ib.firstChild);
						}
					} else {
						aD.setStart(aB, aB.nodeValue.length);
						aD.setEnd(fp, 0);
					}
				}
			}
		...
		jG : function(el) {
			...
			if (kL) {
				if (el == aB) {
					if (ft) {
						el.nodeValue = el.nodeValue.substring(0, cL)
								+ this.jh(el.nodeValue.substring(cL));
					} else {
						bi[bi.length] = el;
					}
				} else if (el == fp) {
					if (ft) {
						el.nodeValue = this.jh(el.nodeValue.substring(0, cO))
								+ el.nodeValue.substring(cO);
					} else {
						bi[bi.length] = el;
						for ( var i = 0; i < bi.length; i++) {
							...
							if (i == 0) {
								...
								if ((ao.tagName.toUpperCase() == "SPAN")
										&& (bi[i].nodeValue.substring(cL) == ao.innerHTML)) {
									this.hx(ao);
								} else {
								...
								var ao = bi[i].parentNode;
								if ((ao.tagName.toUpperCase() == "SPAN")
										&& (bi[i].nodeValue.substring(0, cO) == ao.innerHTML)) {
									this.hx(ao);
								} else {
		...
		jh : function(bM) {
			...
			case "lowercase":
				...
				break;
			case "size":
				return "<span id=\"eWebEditor_Temp_Span_FontSize\" style=\"font-size:"
						+ je + "\">" + bM + "</span>";
				break;
			}
		},
...
kQ.Execute = function() {
	...
	if (!this.hM) {
		...
		if (gz.x != 0) {
			bQ.style.left = (-1 * gz.x) + "px";
		}
		if (gz.y != 0) {
			bQ.style.top = (-1 * gz.y) + "px";
		}
		this.hM = true;
	} else {
...
function oX() {
	...
	if (F.as) {
		if (config.AutoDetectPaste == "1") {
			...
			var an = aZ.GetClipboardFlag();
			var fB = an.split("|");
			if (fB[5] == "1") {
				window.setTimeout("kF()", 10);
				return false;
			} else if (fB[5] == "2") {
				window.setTimeout("uH()", 10);
				return false;
			} else if (fB[5] == "3" || fB[5] == "4") {
				mB();
				window.setTimeout("localUpload()", 10);
				return false;
			} else if (fB[0] == "1" || fB[3] == "1") {
				window.setTimeout("tG('" + an + "')", 10);
				return false;
			}
		}
		mB();
...
function tG(an) {
	if (config.AutoDonePasteOption) {
		aK("pasteoption.htm?autodone=1&flag=" + an, true, true, true);
	} else {
...
function kF() {
	if (F.as) {
		if (config.AutoDonePasteWord) {
			aK("importword.htm?action=paste&autodone=1", true, true, true);
		} else {
			aK("importword.htm?action=paste", true);
		}
	} else {
		aK("pastegecko.htm?action=word");
	}
};
function uH() {
	if (config.AutoDonePasteExcel) {
		aK("importexcel.htm?action=paste&autodone=1", true, true, true);
	} else {
...
function ix() {
	EWEB.Focus();
	if (F.as) {
		var V = fn(clipboardData.getData("Text"));
		insertHTML(V);
	} else {
		...
		for ( var i = 0; i < K.length; i++) {
			...
			if (this.cV) {
				...
				el.className = el.className.replace(
						/(^| )ewebeditor__showtableborders/gi, '');
			}
		}
...
var jK = {
	...
	eD : function() {
		...
		if (this.cV) {
			...
			dK.className = dK.className.replace(
					/(^| )ewebeditor__showblocks/gi, '');
		}
...
var vp = (function() {
	...
	var BZ = function(CG) {
		...
		case 4:
			...
			break;
		case 5:
			aQ = aQ.substring(3, 5) + aQ.substring(1, 3) + "0"
					+ aQ.substring(0, 1);
			break;
		case 6:
...
R.hh = function(kG) {
	kG.style.cssText = "margin:0;" + "padding:0;" + "border:0;"
...
R.qJ = function(hS) {
	if (hS.document) {
		hS.document.parentWindow = hS;
	}
	for ( var i = 0; i < hS.frames.length; i++) {
		R.qJ(hS.frames[i]);
	}
};
R.lJ = function() {
...
R.hg = function(w, node) {
	...
	var hn = R.mh(bN);
	while (bN
			&& !(hn == w && (bN == w.document.body || bN == w.document.documentElement))) {
	...
	if (R.ce(w.document.body, 'position') != 'static'
			|| (F.as && R.vN(node) == null)) {
...
R.az = function(jP, pY, v_Listener) {
	if (F.as) {
		jP.attachEvent("on" + pY, v_Listener);
	} else {
		jP.addEventListener(pY, v_Listener, false);
	}
};
R.hf = function(jP, pY, v_Listener) {
	if (F.as) {
		jP.detachEvent("on" + pY, v_Listener);
	} else {
		jP.removeEventListener(pY, v_Listener, false);
	}
};
R.eF = function(vY) {
	if (F.as) {
...
R.rN = function(kG, qs) {
	if (F.as && !F.BX) {
		...
		kG.style.filter = (qs > 100 ? ""
				: "progid:DXImageTransform.Microsoft.Alpha(opacity=" + qs + ")");
	} else {
		kG.style.opacity = qs;
	}
};
R.ce = function(kG, hO) {
	if (F.as) {
		hO = hO.replace(/\-(\w)/g, function(CC, p1) {
			return p1.toUpperCase();
		});
		return kG.currentStyle[hO];
	} else {
		try {
			return kG.ownerDocument.defaultView.getComputedStyle(kG, "")
					.getPropertyValue(hO);
		} catch (e) {
		}
	}
...
C.lF = function(eE) {
	...
	if (C.ai() != "Control") {
		...
		while (eE.IndexOf(el.tagName.toUpperCase()) < 0) {
			if (el.tagName.toUpperCase() == "BODY") {
				el = null;
				break;
			}
...
C.ai = function() {
	if (F.as) {
		...
		if (bk && bk.rangeCount == 1) {
			var range = bk.getRangeAt(0);
			if (range.startContainer == range.endContainer
					&& (range.endOffset - range.startOffset) == 1
					&& range.startContainer.nodeType == 1
					&& At[range.startContainer.childNodes[range.startOffset].nodeName
							.toLowerCase()]) {
				type = 'Control';
			}
		}
...
C.zp = function(sg) {
	if (F.as) {
		...
		while (cr) {
			if (cr.nodeType == 1 && cr.nodeName.IEquals(sg))
				return true;
			cr = cr.parentNode;
		}
		return false;
...
C.Save = function(Ed) {
	...
	if (Bd) {
		...
		if (nr) {
			if (nr.parentElement && R.dV(nr.parentElement()) != J) {
				nr = null;
			} else if (nr.item && R.dV(nr.item(0)) != J) {
				nr = null;
			}
		}
	}
...
C.ww = function(el) {
	if (F.as) {
		...
		if (iP.inRange(gF)) {
			...
			if (((iP.compareEndPoints("StartToEnd", gF) < 0) && (iP
					.compareEndPoints("StartToStart", gF) > 0))
					|| ((iP.compareEndPoints("EndToStart", gF) > 0) && (iP
							.compareEndPoints("EndToEnd", gF) < 0))) {
				return true;
			}
		}
		return false;
		...
		if ((gF.compareBoundaryPoints(Range.START_TO_END, iP) >= 0)
				&& (gF.compareBoundaryPoints(Range.END_TO_START, iP) <= 0)) {
			return true;
		}
		return false;
	}
	...
	var vf = function() {
		...
		if (F.as) {
			...
			for ( var i = 0; i < K.length; i++) {
				...
				if ((fG.compareBoundaryPoints(Range.START_TO_END, ad) >= 0)
						&& (fG.compareBoundaryPoints(Range.END_TO_START, ad) <= 0)) {
...
function getSelectedHTML() {
	...
	case "EDIT":
		if (window.getSelection) {
			...
			return EWEB.T.selection.createRange().htmlText;
		}
		break;
	case "TEXT":
		var fV = EWEB.db;
...
config.Toolbars = [ [ "TBHandle", "FontName", "FontSize", "Bold", "Italic",
		"UnderLine", "TBSep", "OrderedList", "UnOrderedList", "Indent",
		"Outdent", "TBSep", "JustifyLeft", "JustifyCenter", "JustifyRight",
		"TBSep", "ForeColor", "BackColor", "Emot", "HorizontalRule", "TBSep",
		"Maximize", "About" ] ];
